  ...
  if (unlinkConfirm) {
    return (
      <tr className="border-b border-[#F0F0F4]">
        <td colSpan={showBulkCheckbox ? 10 : 9} className="px-4 py-3">
...
export default function MandatePanel() {
  ...
  const eligibleIds = useMemo(
    () => filteredCandidates
      .filter((mc) => mc.stage === 'CV' && mc.status === 'Internal Review')
      .map((mc) => mc.id),
    [filteredCandidates]
  )
  ...
  async function handleSubmitToClient() {
    ...
    const { error } = await supabase
      .from('mandate_candidates')
      .update({ status: 'Processed - FB Pending', status_changed_at: new Date().toISOString() })
      .in('id', ids)
    if (error) return
    const toLog = mandateCandidates.filter((mc) => selectedIds.has(mc.id))
    for (const mc of toLog) {
      await logActivity({
        ...
        changeType: 'status',
        oldValue: 'Internal Review',
        newValue: 'Processed - FB Pending',
      })
    }
            ...
            {!isRecruiter && selectedIds.size > 0 ? (
              <div className="flex items-center gap-3 ml-auto">
                ...
                <button
                  ...
                  className="h-7 px-3 rounded-lg text-xs font-semibold text-white transition hover:opacity-90"
                  style={{ backgroundColor: '#5E6AD2' }}
                >
                  Submit to Client
                </button>
